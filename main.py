...
def _stooq_daily_closes(symbol: str) -> List[Tuple[datetime, float]]:
    ...
    for attempt in range(3):
        try:
            r = _requests_get(url, params=params, timeout=12, headers=UA_HEADERS)
...
def _finnhub_quote(symbol: str) -> dict:
    ...
    r = _requests_get(url, params=params, timeout=10, headers=UA_HEADERS)
    if r.status_code == 429:
...
def news_briefing(
    ...
    google_syms = syms[YAHOO_TICKERS_MAX: min(len(syms), 15)]
