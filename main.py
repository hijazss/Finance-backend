...
def normalize_fund_name(name: str) -> str:
    n = name.lower()
    for group in SOVEREIGN_FUNDS.values():
        for alias in group:
            if alias in n:
...
@app.get("/report/sovereign/today")
def sovereign_report_today():
    if not QUIVER_TOKEN:
        return {"error": "Missing Quiver token"}

    url = "https://api.quiverquant.com/beta/live/institutionaltrading"
    ...
    if resp.status_code != 200:
        return {"error": "Quiver request failed"}
    ...
    for r in rows:
        ...
        for fund_aliases in SOVEREIGN_FUNDS.values():
            for alias in fund_aliases:
                if alias in institution:
                    matched = True
                    canonical_name = fund_aliases[0].title()
                    break
            if matched:
                break
        ...
        shares_change = r.get("SharesChange", 0)
        ...
        signals.append({
            ...
            "fundName": canonical_name,
            "action": action,
            "filedAt": r.get("ReportDate"),
            "details": (
                "New position" if shares > 0 and shares_change == shares
                else "Increase" if shares_change > 0
                else "Decrease"
            )
        })

    signals.sort(
        key=lambda x: x.get("filedAt") or "",
        reverse=True
    )
